...
<Card.Root class="w-full">
	<Card.Header>
		<Card.Title class="text-2xl">{$_('auth.login')}</Card.Title>
	</Card.Header>
	<Card.Content>
		<form method="POST" use:enhance>
			<Form.Field {form} name="username">
				<Form.Control>
					{#snippet children({ props })}
						<Form.Label>{$_('auth.username')}</Form.Label>
			...
			<Form.Field {form} name="password">
				<Form.Control>
					{#snippet children({ props })}
						<Form.Label>{$_('auth.password')}</Form.Label>
		...
		<div class="mt-4 text-center text-sm">
			{$_('auth.no_have_account')}
			<a href="/signup" class="underline"> {$_('auth.signup')} </a>
		</div>
	</Card.Content>
</Card.Root>
